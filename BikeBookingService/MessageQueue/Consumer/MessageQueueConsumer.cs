...
namespace BikeBookingService.MessageQueue.Consumer;
...
public class MessageQueueConsumer : BackgroundService
{
    ...
    private async Task ConsumeMessage(CancellationToken cancellationToken)
    {
        ...
        {
            var receiveMessages = await consumer.ReceiveMessages(_configuration["MessageQueue:BikeTrackingQueue"]);
            foreach (var message in receiveMessages)
            {
                ...
                await consumer.DeleteMessage(_configuration["MessageQueue:BikeTrackingQueue"], message);
            }
        }
    }
}
